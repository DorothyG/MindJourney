mj.controller("paperListCtrl", function ($http, $scope, $stateParams, $state,
                                         handleData, doPagination) {

    var examUrl = "../tr/examResult/";
    var isClick = [];

    $scope.load = function () {
        var idExam = $stateParams.examId;
        $http({
            method: "get",
            url: examUrl + "datalist",
            params: {
                idExam: idExam
            }
    ...
    $scope.modify = function (answers) {
        console.log(answers);
        handleData.WithoutData("post", examUrl + "modify", answers)
