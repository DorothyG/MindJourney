mj.controller("classBeforeCtrl", function ($http, $scope, handleData) {
    ...
    $scope.loadAllData = function () {
        ...
        promise.then(function (success) {
            $scope.classBefores = success.rows;
    ...
    $scope.checkTr = function ($event, tr) {
        if ($($event.target).is("td")) {
            var checkbox = !$scope.Checked(tr.classBefore._id);
            var action = (checkbox ? 'add' : 'remove');
            updateSelected(action, tr.classBefore._id, "isSelected");
        }
    };
    ...
    $scope.paginating = function (page) {
        ...
        if (typeof page === 'number' && !isNaN(page)) {
            $scope.index = page;
            $scope.classBefores = doPagination.paginateAll(rowsAll, page);
        } else if (page == "<<") {
            $scope.classBefores = doPagination.paginateAll(rowsAll, (page - 1));
        } else if (page == ">>") {
            $scope.classBefores = doPagination.paginateAll(rowsAll, (page + 1));
        }
        //分页加载数据
