mj.controller("classAfterCtrl", function($http, $scope, $state,
                                         handleData){
    ...
    $scope.loadAllData = function () {
        ...
        promise.then(function (success) {
            $scope.classAfters = success.rows;
    ...
    $scope.checkTr = function ($event, tr) {
        if ($($event.target).is("td")) {
            var checkbox = !$scope.Checked(tr.classAfter._id);
            var action = (checkbox ? 'add' : 'remove');
            updateSelected(action, tr.classAfter._id, "isSelected");
        }
    };
    ...
    $scope.paginating = function (page) {
        ...
        if (typeof page === 'number' && !isNaN(page)) {
            $scope.index = page;
            $scope.classAfters = doPagination.paginateAll(rowsAll, page);
        } else if (page == "<<") {
            $scope.classAfters = doPagination.paginateAll(rowsAll, (page - 1));
        } else if (page == ">>") {
            $scope.classAfters = doPagination.paginateAll(rowsAll, (page + 1));
        }
        //分页加载数据
    ...
    $scope.correct = function (classAfter) {
        $state.go("index.classAfterList", {practiceId: classAfter.idPractice._id});
    }
});
