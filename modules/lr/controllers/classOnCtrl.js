mj.controller("classOnCtrl", function($http, $scope, $state,
                                      handleData){
    ...
    $scope.checkTr = function ($event, tr) {
        if ($($event.target).is("td")) {
            var checkbox = !$scope.Checked(tr.classOn._id);
            var action = (checkbox ? 'add' : 'remove');
            updateSelected(action, tr.classOn._id, "isSelected");
        }
    };
    ...
    $scope.paginating = function (page) {
        ...
        if (typeof page === 'number' && !isNaN(page)) {
            $scope.index = page;
            $scope.classOns = doPagination.paginateAll(rowsAll, page);
        } else if (page == "<<") {
            $scope.classOns = doPagination.paginateAll(rowsAll, (page - 1));
        } else if (page == ">>") {
            $scope.classOns = doPagination.paginateAll(rowsAll, (page + 1));
        }
        //分页加载数据
    ...
    $scope.transfer = function (classOn) {
        $state.go("index.practiceList", {practiceCode: classOn.idPractice.practiceCode});
    }

    $scope.correct = function (classOn) {
        $state.go("index.classOnList", {practiceId: classOn.idPractice._id});
    }
});
