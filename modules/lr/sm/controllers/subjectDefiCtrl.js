mj.controller("subjectDefiCtrl", function ($http, $scope, handleData) {
    ...
    $scope.checkTr = function ($event, tr) {
        if ($($event.target).is("td")) {
            var checkbox = !$scope.Checked(tr.subject._id);
            var action = (checkbox ? 'add' : 'remove');
            updateSelected(action, tr.subject._id, "isSelected");
        }
    };
    ...
    $scope.paginating = function (page) {
        ...
        if (typeof page === 'number' && !isNaN(page)) {
            $scope.index = page;
            $scope.subjects = doPagination.paginateAll(rowsAll, page);
        } else if (page == "<<") {
            $scope.subjects = doPagination.paginateAll(rowsAll, (page - 1));
        } else if (page == ">>") {
            $scope.subjects = doPagination.paginateAll(rowsAll, (page + 1));
        }
        //分页加载数据
