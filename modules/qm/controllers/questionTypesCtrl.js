mj.controller("questionTypesCtrl", function($http, $scope, handleData){
    $(function () {
        //模态框初始化
        $(".modal").modal();
        //去掉幕布
    ...
    $scope.checkTr = function ($event, tr) {
        if ($($event.target).is("td")) {
            var checkbox = !$scope.Checked(tr.type._id);
            var action = (checkbox ? 'add' : 'remove');
            updateSelected(action, tr.type._id, "isSelected");
        }
    };
    ...
    $scope.paginating = function (page) {
        ...
        if (typeof page === 'number' && !isNaN(page)) {
            $scope.index = page;
            $scope.types = doPagination.paginateAll(rowsAll, page);
        } else if (page == "<<") {
            $scope.types = doPagination.paginateAll(rowsAll, (page - 1));
        } else if (page == ">>") {
            $scope.types = doPagination.paginateAll(rowsAll, (page + 1));
        }
        //分页加载数据
