mj.controller("dataHandleCtrl", function ($http, $scope, $state,
                                          handleData) {
    ...
    $scope.checkTr = function ($event, tr) {
        if ($($event.target).is("td")) {
            var checkbox = !$scope.Checked(tr.rule._id);
            var action = (checkbox ? 'add' : 'remove');
            updateSelected(action, tr.rule._id, "isSelected");
        }
    };
    ...
    $scope.paginating = function (page) {
        ...
        if (typeof page === 'number' && !isNaN(page)) {
            $scope.index = page;
            $scope.rules = doPagination.paginateAll(rowsAll, page);
        } else if (page == "<<") {
            $scope.rules = doPagination.paginateAll(rowsAll, (page - 1));
        } else if (page == ">>") {
            $scope.rules = doPagination.paginateAll(rowsAll, (page + 1));
        }
        //分页加载数据
    ...
    $scope.transfer = function (rule) {
        $scope.view = rule;
    }

    $scope.modi = function (view) {
        $state.go("index.dataHandleCreate", {ruleId: view.ruleCode});
    }

    $scope.add = function () {
        $state.go("index.dataHandleCreate", {ruleId: 0});
    }
});