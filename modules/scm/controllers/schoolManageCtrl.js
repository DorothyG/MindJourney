mj.controller("schoolManageCtrl", function ($http, $scope, handleData) {
    $(function () {
        //模态框初始化
        $('.modal').modal();
        //解除幕布
    ...
    $scope.checkTr = function ($event, tr) {
        if ($($event.target).is("td")) {
            var checkbox = !$scope.Checked(tr.school._id);
            var action = (checkbox ? 'add' : 'remove');
            updateSelected(action, tr.school._id, "isSelected");
        }
    };
    ...
    $scope.paginating = function (page) {
        ...
        if (typeof page === 'number' && !isNaN(page)) {
            $scope.index = page;
            $scope.schools = doPagination.paginateAll(rowsAll, page);
        } else if (page == "<<") {
            $scope.schools = doPagination.paginateAll(rowsAll, (page - 1));
        } else if (page == ">>") {
            $scope.schools = doPagination.paginateAll(rowsAll, (page + 1));
        }
        //分页加载数据
