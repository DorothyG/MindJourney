mj.controller("studentInfoCtrl", function ($http, $scope, handleData, doPagination) {
    ...
    $scope.checkTr = function ($event, tr) {
        if ($($event.target).is("td")) {
            var checkbox = !$scope.Checked(tr.student._id);
            var action = (checkbox ? 'add' : 'remove');
            updateSelected(action, tr.student._id, "isSelected");
        }
    };
    ...
    $scope.paginating = function (page) {
        ...
        if (typeof page === 'number' && !isNaN(page)) {
            $scope.index = page;
            $scope.students = doPagination.paginateAll(rowsAll, page);
        } else if (page == "<<") {
            $scope.students = doPagination.paginateAll(rowsAll, (page - 1));
        } else if (page == ">>") {
            $scope.students = doPagination.paginateAll(rowsAll, (page + 1));
        }
        //分页加载数据
...
//新建一个学生信息
    $scope.create = function (add) {
        var promise = handleData.createUser("post", commonUrl + "create", add, 2);
        promise.then(function (success) {
